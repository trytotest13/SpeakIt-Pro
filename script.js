(function(){
  ...
  // Shared promise-backdrop pattern: open() shows the backdrop and resolves
  // with whatever close(result) is later called with.
  function dialogOpener(backdropEl){
    let resolve = null;
    const close = (result)=>{
  ...
  const confirmDlg = dialogOpener(confirmBackdrop);

  function confirmDialog(message){
    confirmMessageEl.textContent = message;
    return confirmDlg.open();
  }
  confirmOkBtn.addEventListener('click', ()=>confirmDlg.close(true));
  confirmCancelBtn.addEventListener('click', ()=>confirmDlg.close(false));
  confirmBackdrop.addEventListener('click', (e)=>{ if(e.target === confirmBackdrop) confirmDlg.close(false); });
  ...
  const editDlg = dialogOpener(editBackdrop);

  function editDialog(initialText){
    editTaskInput.value = initialText;
    const p = editDlg.open();
    setTimeout(()=>{ editTaskInput.focus(); editTaskInput.select(); }, 50);
    return p;
  }
  editSaveBtn.addEventListener('click', ()=>editDlg.close(editTaskInput.value));
  editCancelBtn.addEventListener('click', ()=>editDlg.close(null));
  editBackdrop.addEventListener('click', (e)=>{ if(e.target === editBackdrop) editDlg.close(null); });
  editTaskInput.addEventListener('keydown', (e)=>{
    if(e.key === 'Enter'){ e.preventDefault(); editDlg.close(editTaskInput.value); }
    if(e.key === 'Escape'){ editDlg.close(null); }
  });
  ...
  function attachLongPress(el, onFire, shouldSkip){
    let pressTimer = null;
    ...
    ['pointerup','pointercancel','pointerleave'].forEach(evt=>{
      el.addEventListener(evt, ()=>{ if(!dragging) cancelPress(); });
    });
  }
  ...
  function enableDragReorder(container, list){
    container.querySelectorAll('.msg-row').forEach(rowEl=>{
      ...
      attachLongPress(bubble,
        ...
        (e)=> selectionMode || (e.target.closest && e.target.closest('.check, .act-btn')));
    });
  }

  // Shared pointer-track reorder: move rowEl with the pointer, then commit().
  function trackReorder(rowEl, container, rowSel, commit){
    ...
    const onMove = (e)=>{
      e.preventDefault();
      const y = e.clientY;
      const afterElement = getDragAfterElement(container, y, rowSel);
  ...
  function beginDrag(rowEl, container, list){
    trackReorder(rowEl, container, '.msg-row:not(.reordering)', ()=>commitOrder(container, list));
  }

  function getDragAfterElement(container, y, rowSel){
    const els = [...container.querySelectorAll(rowSel)];
  ...
  function commitDomOrder(container, rowSel, key, setOrder){
    [...container.querySelectorAll(rowSel)].forEach((rowEl, i)=>setOrder(rowEl.dataset[key], i));
    saveState();
    render();
  }

  function commitOrder(container, list){
    commitDomOrder(container, '.msg-row', 'taskId', (id, i)=>{
  ...
  function enableListDragReorder(container){
    container.querySelectorAll('.chat-row').forEach(rowEl=>{
      attachLongPress(rowEl, ()=>beginListDrag(rowEl, container));
    });
  }
  ...
  function beginListDrag(rowEl, container){
    ...
    trackReorder(rowEl, container, '.chat-row:not(.reordering)', ()=>{
  ...
  function commitListOrder(container){
    const profile = activeProfile();
    commitDomOrder(container, '.chat-row', 'listId', (id, i)=>{
      const list = profile.lists.find(l=>l.id===id);
  ...
  function openContextMenu(x, y, listId, taskId){
    ...
    const items = [
      ...
      {label: task.pinned ? 'Unpin' : 'Pin', icon:PIN_ICON_PATH, accent: !task.pinned, action:()=>togglePin(listId, taskId)},
  ...
  function showListModal({mode, targetId = null, title, btn, placeholder, value = '', color = COLORS[0], selectText = false}){
    modalMode = mode;
    modalTargetId = targetId;
    document.getElementById('modalTitle').textContent = title;
    if(placeholder !== undefined) document.getElementById('newListName').placeholder = placeholder;
    document.getElementById('createListBtn').textContent = btn;
    document.getElementById('newListName').value = value;
  ...
  function openModal(){
    showListModal({mode:'list-create', title:'New list', btn:'Create', placeholder:'e.g. Groceries'});
  }
  ...
  function openRenameModal(){
    ...
    const list = profile.lists.find(l=>l.id===profile.activeListId);
    if(!list) return;
    showListModal({mode:'list-rename', title:'Rename list', btn:'Save', value:list.name, color:list.color, selectText:true});
  }

  function openProfileCreateModal(){
    showListModal({mode:'profile-create', title:'New profile', btn:'Create', placeholder:'e.g. Job tasks, YouTube videos'});
  }

  function openProfileRenameModal(profileId){
    const profile = state.profiles.find(p=>p.id===profileId);
    if(!profile) return;
    showListModal({mode:'profile-rename', targetId:profileId, title:'Rename profile', btn:'Save', value:profile.name, color:profile.color, selectText:true});
  }
